...
bookController.discountBook = catchAsync(async (req, res, next) => {
    ...
    if (!bookId || discountRate == null) {
      throw new AppError(400, "Missing required fields", "Discount Book Error");
    }
  
    // Find book by ID
    const book = await Book.findById(bookId);
    if (!book) {
      throw new AppError(404, "Book not found", "Discount Book Error");
    }
    ...
    sendResponse(res, 200, true, book, null, "Discount applied successfully");
});
...
bookController.deleteDiscountBook = catchAsync(async (req, res, next) => {
    ...
    const book = await Book.findById(bookId);
    if (!book) {
      throw new AppError(404, "Book not found", "Delete Discount Error");
    }
    ...
    sendResponse(res, 200, true, null, null, "Discount removed successfully");
  });
...
bookController.getDiscountBook = catchAsync(async (req, res, next) => {
    try {
      ...
      const discountedBooks = await Book.aggregate([
        {
          $match: {
            ...
            isDeleted: false,     // Chỉ lấy những sách không bị xóa
          },
        },
        {
          $lookup: {
            from: "bookcategories", // Kết nối với bảng bookcategories
      ...
      if (!discountedBooks || discountedBooks.length === 0) {
        throw new AppError(404, "No discounted books found", "Get Discounted Books Error");
      }
  
      // Gửi kết quả về client
      sendResponse(res, 200, true, discountedBooks, null, "Discounted books retrieved successfully");
    } catch (error) {
      // Xử lý lỗi khi giá trị _id không hợp lệ hoặc các lỗi khác
      if (error instanceof mongoose.Error.CastError && error.kind === "ObjectId") {
        next(new AppError(400, "Invalid ObjectId format", "Get Discounted Books Error"));
      } else {
        next(error);
      }
    }
